...
$post =mysqli_fetch_assoc($select_post);
    ...
    if (empty($comment)) {
        ...
        $insert = mysqli_query($conn  , "insert into comments (patent_id , user_name,comment,com_date,seen_status) values ('$id','$username','$comment' ,'$date',0 )");
        if(isset($insert)) {
...
?>
    ...
    <article class="col-md-9 col-lg-9 " xmlns="http://www.w3.org/1999/html">
        <div class="col-md-12" art_bg >
            ...
            <div class="cate_post" style="padding: 5px; margin: 10px 0;background: white;border: solid 1px   #E7E7E7;">
                ...
                <div class="col-md-12">
                    ...
                    <?php
                    if($post['image'] == ''){}else{echo '<img src="../tisc/'.$post['image'].'" width="100%" style="max-height: 500px;"/>';}
                    ?>
                </div>
                <div class="col-md-12">
                    <div class="col-md-12" style="margin: 15px;background: #f3dcd9; font-size:20px;">
                    <p class="pull-right"><i class="fa fa-user">   اسم المخترع : <?php echo $post['inventor_name']; ?></i> | <i class="fa fa-clock-o">&ensp;</i> تاريخ الايداع :  <?php echo $post['date']; ?></p&ensp;|&ensp;>
                    <p class=""><i class="fa fa-info" aria-hidden="true"></i>حالة البراءة : <?php echo $post['category']; ?></p>
                    </div>
                    <div class="col-md-12">
                        <hr/>
        ...
        $sel_user2 = mysqli_query($conn , 'Select * from main_users where user_id = 1');
        $main_user = mysqli_fetch_assoc($sel_user2);
        $comm = mysqli_query($conn , "select * from comments where (patent_id = '$id' and user_name = '$sub_main_user[username]') or (patent_id = '$id' and user_name = '$main_user[username]')   order by com_date desc");
