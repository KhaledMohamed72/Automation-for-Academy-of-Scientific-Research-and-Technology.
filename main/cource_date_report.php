...
$reslut = mysqli_query($conn, "SELECT * FROM main_users WHERE username = '$username' and password = '$password'");
...
<div class="text-center">
    <p style="text-decoration: underline;font-size: 20px"><strong>الدورات التدريبية المسجلة من تاريخ <?php echo $date['from_date'] . ' الي ' . $date['to_date']; ?></strong></p>
    <br>
    <table class="table table-bordered">
        <tbody>
        <tr style="background-color: #e4f1f7">
            <th scope="row">&ensp; اسم الدورة</th>
            <td class="text-center">تاريخ الدورة</td>
            <td class="text-center">الجهة المانحة</td>
            <td class="text-center">ملاحظة</td>
        </tr>
        </tbody
        <?php

        $patents = mysqli_query($connect ,"select * from cources where date between '$date[from_date]' and '$date[to_date]' ORDER BY date DESC");
