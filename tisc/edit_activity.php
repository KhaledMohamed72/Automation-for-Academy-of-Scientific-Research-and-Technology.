...
$get_post = mysqli_query($conn,"SELECT * FROM activities WHERE cource_id= '$_GET[post]'");
$post = mysqli_fetch_assoc($get_post);
if(isset($_GET['delete'])){

    $sql =mysqli_query($conn, "UPDATE activities SET image = '' WHERE cource_id = '$_GET[delete]'");
            ...
            if (isset($_FILES['image']['name'])) {
                ...
                foreach($_FILES['image']['name'] as $key=>$val){
                    ...
                    if (in_array($ext, $supported_image)) {
                        ...
                        $insert = mysqli_query($conn, "UPDATE activities SET cource_name = '$cource_name' , donor ='$donor' , note='$note' , image='$image_db' , date = '$date'  WHERE cource_id='$_GET[post]'");

                        if(isset($insert)){

                            $msg = '<div class="alert alert-info" role="alert">تم تعديل النشاط بنجاح جاري تحويلك الي الانشطة</div>';

                            echo  '<meta http-equiv="refresh" content="3; \'activities.php\' "/>';
                        ...
                        $insert = mysqli_query($conn, "UPDATE activities SET cource_name = '$cource_name' , donor ='$donor' , note='$note' , date = '$date'  WHERE cource_id='$_GET[post]'");
                    ...
                    <div class="panel-body">
                        <form action="" method="post" enctype="multipart/form-data" class="form-horizontal">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-5">
                                    <input type="text" class="form-control" id="cource_name" name="cource_name" value="<?php echo $post['cource_name'];?>" placeholder="أدخل اسم النشاط">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-5">
                                    <input type="text" class="form-control" id="donor" name="donor" value="<?php echo $post['donor'];?>" placeholder="أدخل المحاضر">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="col-sm-offset-2 col-sm-10">
                                    <button type="submit" name="add_post" class="btn btn-danger">تعديل النشاط</button>
                                </div>
                            </div>
                        </form>
                    </div>
